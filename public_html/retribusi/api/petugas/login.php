...
// 	}
	...
	$query = "SELECT petugas.userid,petugas.users_id,petugas.profile_pic,petugas.foto_ktp,petugas.email,petugas.telp, petugas.password from petugas where petugas.name ='$username'";
	if ($result = mysqli_query($con, $query)) {
		...
		if (mysqli_num_rows($result) > 0) {
	        ...
	        if (password_verify( $password,$data['password'])) {
    		    ...
    		    $response["phone_number"] = $data['telp'];
    		    $response["id_picture"] = $data['foto_ktp'];
		    ...
		    $query = "SELECT id_pedagang,profile_pic,telp,email,foto_ktp, password from pedagang where name ='$username'";
	        if ($result = mysqli_query($con, $query)) {
	            $response['pasar'] = array();
	            $response['list_menu'] = array();
		        if (mysqli_num_rows($result) > 0) {
        	        ...
        	        if (password_verify( $password,$data['password'])) {
        	            $response["username_exist"] = 1;
            		    $response["user_id"] = $data['id_pedagang'];
            		    $response["username"] = $username;
            		    $response["photo"] = $data['profile_pic'];
            		    $response["email"] = $data['email'];
    		    $response["phone_number"] = $data['telp'];
